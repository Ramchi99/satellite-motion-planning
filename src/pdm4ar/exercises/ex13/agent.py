...
class SatelliteAgent(Agent):
    ...
    def on_episode_init(self, init_sim_obs: InitSimObservations):
        ...
        # Precompute impulse-preserving commands for fast lookup in get_commands
        self.precomputed_cmds = []
        dt = 0.1
        # Determine the duration of the planned trajectory
        t_final = self.cmds_plan.timestamps[-1]
        # Calculate number of steps needed, adding a small buffer
        num_steps = int(np.ceil(t_final / dt)) + 10

        for i in range(num_steps):
            t = i * dt
            # Compute the command for the interval [t, t + dt]
            cmd = self._get_impulse_preserving_command(t, t_sim_step=dt)
            self.precomputed_cmds.append(cmd)
    ...
    def get_commands(self, sim_obs: SimObservations) -> SatelliteCommands:
        ...
        # --- NEW IMPULSE PRESERVING METHOD ---
        # We integrate the plan from t_now to t_now + 0.1s
        # cmds = self._get_impulse_preserving_command(sim_obs.time, t_sim_step=0.1)

        # Fast lookup of precomputed commands
        dt = 0.1
        idx = int(round(float(sim_obs.time) / dt))
        #print(f"Time: {sim_obs.time:.2f} s, Index: {idx}")

        if idx < len(self.precomputed_cmds):
            cmds = self.precomputed_cmds[idx]
        else:
            # Default to zero thrust if we are past the precomputed plan
            cmds = SatelliteCommands(F_left=0.0, F_right=0.0)
    ...
    def _get_impulse_preserving_command(self, t_start_raw, t_sim_step: float = 0.1) -> SatelliteCommands:
        """
        Calculates the average thrust required over the next simulation step 
        to match the impulse (area) of the planned trajectory.
        Uses Simpson's 1/3 rule for integration accuracy.
        """
        t_start = float(t_start_raw)

        t_mid = t_start + t_sim_step / 2
        t_end = t_start + t_sim_step

        # Sample the plan at Start, Middle, and End of the simulation window
        # We use .at_interp to get the FOH ramp values
        u_start = self.cmds_plan.at_interp(t_start)
        u_mid   = self.cmds_plan.at_interp(t_mid)
        u_end   = self.cmds_plan.at_interp(t_end)

        # Simpson's Rule: Average Height = (Start + 4*Mid + End) / 6
        avg_F_left  = (u_start.F_left + 4 * u_mid.F_left + u_end.F_left) / 6.0
        avg_F_right = (u_start.F_right + 4 * u_mid.F_right + u_end.F_right) / 6.0

        return SatelliteCommands(F_left=avg_F_left, F_right=avg_F_right)
